...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/scrivener.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      app: {
        files: [{
          ...
          dest: '<%= config.dist %>/scrivener.app'
        }]
      },
      zipToApp: {
        files: [{
  ...
  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/scrivener.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/scrivener.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/scrivener.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/scrivener.app/Contents/MacOS/node-webkit', '555');
  });
  ...
  grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
    var done = this.async();
    var createDmgCommand = 'resources/mac/package.sh "scrivener"';
